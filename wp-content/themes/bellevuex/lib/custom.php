...
if(!get_option( 'themo_mphb_default_settings', false )){
    ...
    update_option('mphb_confirmation_mode', 'payment'); // Confirmation Mode
...
function th_masterslider_update_check() {
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://import.themovation.com/live-plugin-updater/hotel-booking.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://import.themovation.com/live-plugin-updater/hotel-booking-woocommerce-payments.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
...
function th_register_required_plugins() {
	...
	$plugins = array(
        ...
        array(
            ...
            'source'    => 'http://out.themovation.com/th-widget-pack',
            'required'  => true,
        ),
        array(
        ...
        array(
            'name' => 'Envato Market',
            'slug' => 'envato-market',
            'source' => 'http://out.themovation.com/envato-market',
            'required' => true,
        ),
	);
